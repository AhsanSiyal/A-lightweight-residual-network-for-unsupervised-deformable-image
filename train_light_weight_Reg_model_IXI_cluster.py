...
from arch.light_weight_Reg_model import ResUnet as ResNet
...
def main():
    batch_size = 1
    atlas_dir = '/scratch/ahsan/trans_DIR/IXI_data/atlas.pkl'
    train_dir = '/scratch/ahsan/trans_DIR/IXI_data/Train_30/'
    val_dir = '/scratch/ahsan/trans_DIR/IXI_data/Val_20/'
    weights = [1, 1]  # loss weights
    save_dir = 'case_4bspl_sample30_ep2500_ncc_{}_diffusion_{}/'.format(weights[0], weights[1])
    ...
    val_loader = DataLoader(val_set, batch_size=1, shuffle=False, pin_memory=True, drop_last=True, num_workers=4)
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                print(eval_dsc.avg)
                dice_list.append(eval_dsc.avg)
        best_dsc = max(eval_dsc.avg, best_dsc)
        dice_avg.append(np.average(dice_list))
        save_checkpoint({
            'epoch': epoch + 1,
        ...
        loss_all.reset()
    writer.close()
    lst = np.array(dice_avg)
    np.save('experiments/case4_val_dice_40_ep2500', lst)
